...
const LangSwitch = () => {
  ...
  const initialLocale = (Array.isArray(params?.lng) ? params?.lng[0] : params?.lng) || siteMetadata.fallbackLanguage;
  const [locale, setLocale] = useState<string>(initialLocale);

  useEffect(() => {
    const currentLng = Array.isArray(params?.lng) ? params?.lng[0] : params?.lng;
    if (currentLng) {
